...
export const restoreFile = uid => (dispatch, getState) => {
  LiveEndpoints.Trash.restoreFile(uid)
    .then(({ data }) => {
      ...
      // Subject to change pending improved API endpoints
      const newCurrentFiles = fileList.map(e => e.uid === uid ? ({ ...e, inTrash: false }) : e)
...
export const deleteFile = uid => (dispatch, getState) => {
  LiveEndpoints.Trash.deleteFile(uid)
    .then(({ data }) => {
      ...
      // Subject to change pending improved API endpoints
      const newCurrentFiles = fileList.map(e => e.uid === uid ? null : e).filter(e => e !== null)
